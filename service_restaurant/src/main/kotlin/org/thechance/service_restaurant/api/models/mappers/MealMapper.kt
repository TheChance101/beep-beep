...
fun Meal.toDto(): MealDto =
    MealDto(id = id, name = name, description = description, price = price, cuisines = cuisines)

fun List<Meal>.toDto(): List<MealDto> = this.map { it.toDto() }

fun MealDto.toEntity(): Meal =
    Meal(id = id, name = name, description = description, price = price, cuisines = cuisines)
